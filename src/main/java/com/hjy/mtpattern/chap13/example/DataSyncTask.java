...
import com.hjy.mtpattern.chap13.*;
...
public class DataSyncTask implements Runnable{
    ...
    private static Connection getConnection()throws Exception{
        ...
        Class.forName("org.mysql.jdbc.JDBCDriver");

        dbConn = DriverManager.getConnection("jdbc:mysql:127.0.0.1:3306/test","root","");
        return dbConn;
    }
    ...
    private static class RecordSaveTask{
        ...
        public RecordSaveTask(int targetFileIndex, String recordDay) {
            this.records = null;
            this.targetFileIndex = targetFileIndex;
    ...
    private SimplePipeline<RecordSaveTask,String> buildPipeline(){
         ...
         *   这里,各个pipe的初始化完全可以在上游Pipe初始化完毕后再初始化其后继Pipe,而不必多个Pipe同时初始化
        ...
        */
            ...
            protected File doProcess(RecordSaveTask task) throws PipeException {
                ...
                if (null==records){
                    ...
                    try {
                        ...
                        throw new PipeException("Failed to save records.",e,this,task);
                    }
                }
                return file;
            }
        ...
        /**
         * 由于这里的几个Pipe都是处理I/O的，为了避免使用锁(以减少上下文切换)但又能保证线程安全
         * 故每个Pipe都采用单线程处理
         * 若各个Pipe要改用线程池来处理，需要注意1)线程安全,2)死锁
        *
        * */
        ...
        final  ThreadPoolExecutor ftpExecutorService = new ThreadPoolExecutor(1, ftpServerConfigs.length, 60,
                TimeUnit.SECONDS, new ArrayBlockingQueue<Runnable>(100), new RejectedExecutionHandler() {
            @Override
        ...
        //将生成的数据文件传输到指定的主机上
        Pipe<File,File> stageTransferFile = new AbsractParallelPipe<File,File, File>(
            ...
            class ParallelTask implements Callable<File>{
                public final Future<FTPClientUtil> ftpUtilHolder;
                public final File file2Transfer;

                public ParallelTask(Future<FTPClientUtil> ftpUtilHolder, File file2Transfer) {
                ...
                public File call() throws Exception {
                    ...
                    ftpUtilHolder.get().upload(file2Transfer);
    ...
    private String[][] retrieveFTPServConf(){
        String[][] ftpServerConfig = new String[][]{
        //        {"192.168.10.105","root","pass"},
      //          {"192.168.10.106","root","pass"},
                {"192.168.10.103","root","pass"},
        };
        ...
        while (rs.next()){
            ...
            if (recordDay.equals(lastRecordDay)){
                ...
                if (null!=lastRecordDay){
                    if (recordCountInTheFile>=1){
                        ...
                        pipeline.process(new RecordSaveTask(targetFileIndex,lastRecordDay));
                    }
            ...
            if (nextTargetFileIndex>targetFileIndex){
                ...
                if (recordCountInTheFile>1){
                    ...
                    pipeline.process(new RecordSaveTask(targetFileIndex,recordDay));
                }
                recordCountInTheFile = 0;
